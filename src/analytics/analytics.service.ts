...
export class AnalyticsService {
  ...
  async getEmailOverview(workspaceId: string) {
    ...
    try {
      ...
      const stats = {
        ...
        totalDelivered: emailLogs.filter((log) => log.status === 'delivered')
          .length,
        totalOpened: emailLogs.filter((log) => log.status === 'opened').length,
        totalClicked: emailLogs.filter((log) => log.status === 'clicked')
          .length,
        totalBounced: emailLogs.filter((log) => log.status === 'bounced')
          .length,
        totalUnsubscribed: emailLogs.filter(
          (log) => log.status === 'unsubscribed',
        ).length,
      };
  ...
  async getCampaignAnalytics(campaignId: string) {
    ...
    try {
      ...
      // 로그 합치고 시간순 정렬
      const allLogs = [...logs, ...eventLogs].sort(
        (a, b) =>
    ...
    const mockActivities = [
      ...
      },
      {
        ...
        details: {
          ...
          clicked_url: 'https://piehands.com',
        },
  ...
  private getEventMessage(
    ...
    const userName = user.properties?.name || user.distinctId || 'Anonymous';
